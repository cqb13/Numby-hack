...
public class TanukiEgapFinder extends Module {
  ...
  private final Setting<Boolean> debug = sgDefault.add(new BoolSetting.Builder()
      .name("debug")
      .description(
          "Useless. Just prints info about every chest it locates in your render distance, will spam chat a lot.")
      .defaultValue(false)
      .build());
      ...
      switch (stage) {
        case 1: {
          ...
          if (block != Blocks.COMPARATOR) {
            ;
            ChatUtils.sendPlayerMsg("/setblock " + adjacent + " " + chest.getY() + " " + chest.getZ()
                + " minecraft:comparator[facing=east]");
          }
          stage++;
          break;
        }
        ...
        case 3: {
          ...
          if (block == Blocks.CHEST) {
            ChatUtils.sendPlayerMsg("/execute if data block " + chest.getX() + " " + chest.getY() + " " + chest.getZ()
                + " Items[{id:\"minecraft:enchanted_golden_apple\"}] as @p run setblock " + chest.getX() + " "
                + chest.getY() + " " + chest.getZ() + " minecraft:diamond_block");
          }
          prevChest = chest;
          stage++;
          break;
        ...
        case 4: {
          ...
          if (diamondPos == Blocks.DIAMOND_BLOCK) {
            ...
            ChatUtils.info((!coords.get() ? Formatting.GREEN + "Found an egap! Wrote coords to file."
                : Formatting.GREEN + "Found an egap! Wrote coords to file. " + prevChest.getX() + " " + prevChest.getY()
                    + " " + prevChest.getZ()));
            writeToFile(prevChest.getX() + " " + prevChest.getY() + " " + prevChest.getZ());
          } else
            ChatUtils.sendPlayerMsg(
                "/setblock " + chest.getX() + " " + chest.getY() + " " + chest.getZ() + " minecraft:air");
          stage++;
          break;
        }
      }
