            ...
            switch (stage) {
                ...
                case 2: {
                    ...
                    if (block != Blocks.ACACIA_LEAVES) {
                        ChatUtils.sendPlayerMsg(
                                "/setblock " + xAdjacent + " " + yAdjacent + " " + chest.getZ()
                ...
                case 3: {
                    ...
                    if (block == Blocks.CHEST) {
                        ChatUtils.sendPlayerMsg(
                                "/execute if data block " + chest.getX() + " " + chest.getY() + " " + chest.getZ()
                                        + " Items[{id:\"minecraft:enchanted_golden_apple\"}] as @p run setblock "
                                        + chest.getX() + " "
                ...
                case 4: {
                    ...
                    if (diamondPos == Blocks.DIAMOND_BLOCK) {
                        ...
                        ChatUtils.info((!coords.get() ? Formatting.GREEN + "Found an egap! Wrote coords to file."
                                : Formatting.GREEN + "Found an egap! Wrote coords to file. " + prevChest.getX() + " "
                                        + prevChest.getY()
                                        + " " + prevChest.getZ()));
                        writeToFile(prevChest.getX() + " " + prevChest.getY() + " " + prevChest.getZ());
                    } else
                        ChatUtils.sendPlayerMsg(
                                "/setblock " + chest.getX() + " " + chest.getY() + " " + chest.getZ()
                                        + " minecraft:air");
                    stage++;
                    break;
                }
